...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    reset()
    # we start off by adding two items to the 0.1 branch. then we move one of
    # the items to the 0.2 branch. Then we expect one item to be in the first
    # branch and one to be in the second. the key is that a new brnach (0.3) has
    # to be created
    ...
    assert(1 in dc.sp.m._(0)._(2).idx.v)
    print('test OK')
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    print('\nmove to major milestone')
    reset()
    # creating two items and moving one to a new milestone
    addMI(0, 1, 'type', 'cat', 'name', 0, 'desc')
    addMI(0, 1, 'type', 'cat', 'name', 0, 'desc')
    editMI(1, 0, 1, 'type', 'cat', 'name', 0, 'desc')
    assert(dc.sp.m._(0).idx.v == {1, 2})
    assert(dc.sp.m._(1).idx.v == {0, 1})
    assert(dc.sp.m._(2).idx.v == {0})
    assert(dc.sp.midx.v[1] == (1, 0))
    assert(dc.sp.midx.v[2] == (0, 1))
    reset()
    # creating only one item and moving it to a new milestone
    addMI(0, 1, 'type', 'cat', 'name', 0, 'desc')
    editMI(1, 0, 1, 'type', 'cat', 'name', 0, 'desc')
    assert(dc.sp.m._(0).idx.v == {1})
    assert(dc.sp.m._(1).idx.v == {0, 1})
    ...
    print('test OK')
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    print('\ntest move away from previous to last item')
    reset()
    addMI(0, 1, 'type', 'cat', 'name', 0, 'desc')
    addMI(0, 2, 'type', 'cat', 'name', 0, 'desc')
    deleteMI(2)
    assert(dc.sp.m._(0).idx.v == {1, 2})
    print('test OK')
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
